...
To develop bitcore-node-bltg:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-bltg.git
...
We now will setup symlinks in `bitcore-node-bltg` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-bltg
ln -s ~/bitcoind-rpc-bltg
```
...
cd devnode
mkdir node_modules
touch bitcore-node-bltg.json
touch package.json
```

Edit `bitcore-node-bltg.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-bltg/bin/bitcore-node-bltg start
```
